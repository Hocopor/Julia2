...
# Проверяем Docker Compose (старый формат: docker-compose или новый: docker compose)
DOCKER_COMPOSE_CMD=""
if command -v docker-compose &> /dev/null; then
    DOCKER_COMPOSE_CMD="docker-compose"
    echo "[OK] Docker Compose (старый формат) установлен"
elif docker compose version &> /dev/null; then
    DOCKER_COMPOSE_CMD="docker compose"
    echo "[OK] Docker Compose (новый формат) установлен"
else
...
if [ "$HAS_API_ACCESS" = true ]; then
    $DOCKER_COMPOSE_CMD -f docker-compose-cloudflare.yml build
else
...
if [ "$HAS_API_ACCESS" = true ]; then
    ...
    $DOCKER_COMPOSE_CMD -f docker-compose-cloudflare.yml up -d
else
...
echo ""
echo "[INFO] Ожидаем запуска (40 секунд)..."
sleep 40
...
echo ""
echo "[INFO] Логи Cloudflare Tunnel:"
$DOCKER_COMPOSE_CMD logs --tail=10 cloudflared

echo ""
echo "========================================================"
echo "[OK] Деплой запущен!"
echo ""

if [ "$HAS_API_ACCESS" = true ]; then
    echo "[OK] DNS настроены автоматически через Cloudflare API"
    echo "[INFO] Подождите 5-30 минут для распространения DNS"
else
...
echo "   • Обновление: $DOCKER_COMPOSE_CMD pull && $DOCKER_COMPOSE_CMD up -d"
echo ""
...
echo "   $DOCKER_COMPOSE_CMD logs cloudflared"
echo "   $DOCKER_COMPOSE_CMD exec app sh"
echo ""
echo "========================================================"